...
class GMSDRCell(torch.nn.Module):
    def __init__(self, device, num_units, input_dim, adj_mx, max_diffusion_step, num_nodes, pre_k, pre_v, nonlinearity='tanh',
        ...
        if filter_type == "laplacian":
            supports.append(graph_algo.calculate_scaled_laplacian(adj_mx, lambda_max=None))
        elif filter_type == "random_walk":
            supports.append(graph_algo.calculate_random_walk_matrix(adj_mx).T)
        elif filter_type == "dual_random_walk":
            supports.append(graph_algo.calculate_random_walk_matrix(adj_mx).T)
            supports.append(graph_algo.calculate_random_walk_matrix(adj_mx.T).T)
        else:
            supports.append(graph_algo.calculate_scaled_laplacian(adj_mx))
