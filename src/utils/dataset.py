...
class Dataset_CaST(Dataset):
    def __init__(self, rootDir, adjFile, scalers, input_dim, output_dim, seq_length_x, seq_length_y, interval, time_delay_scaler):
        self.data = np.load(rootDir) #(b,n,d) [1400,34,6]
        # self.data = np.load(rootDir)['data'] #(b,n,d) [1400,34,6]
        self.time_stamps = self.data.shape[0]
        self.n_nodes = self.data.shape[1]
        
        # adj
        self.dist_adj = np.load(os.path.join(adjFile,'dist_adj.npy')) #[34,34]
        self.sema_adj = np.load(os.path.join(adjFile,'peacor_adj.npy')) #[34,34]
        self.sparse_adj = np.load(os.path.join(adjFile,'sparse_adj.npy')) #[34,34]
    ...
    def _get_edge_attr(self):
        ...
        # edge_attr2[np.isnan(edge_attr2).bool()]=0.0
...
class Dataset_CaST_processed(Dataset):
    def __init__(self, processed_dir, labelFile):
    ...
    #     return ['Graph'+str(fileidx+1)+'.pt' for fileidx in range(self.len())]
